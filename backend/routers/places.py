...
from models import Place, PlaceCreate, PlaceUpdate, Trip
...
async def create_place(trip_id: int, data: PlaceCreate):
...
async def update_place(trip_id: int, place_id: int, data: PlaceUpdate):
...
async def delete_place(trip_id: int, place_id: int):
...
async def reorder_places(trip_id: int, order: str):
    """
    Reorder places within a trip.
    Accepts a comma-separated list of place IDs in the desired order.
    """
    store = get_store()
    ordered_ids = [int(x.strip()) for x in order.split(",") if x.strip()]
    trip = await store.reorder_places(trip_id, ordered_ids)
    if not trip:
        raise HTTPException(404, f"Trip {trip_id} not found")
    return trip
